...
Opta — a macOS SwiftUI app that optimizes PNG files. It shells out to CLI tools (`pngquant`, `oxipng`, `cwebp`) installed via Homebrew. No third-party Swift dependencies.
...
- **ProcessingEngine.swift** — `ObservableObject` that runs the optimization pipeline on a background `DispatchQueue`: quantize via pngquant → optimize via oxipng (PNG) or convert via cwebp (WebP). Uses `Process` to shell out to CLI tools. Supports cancellation via lock-protected flag.
