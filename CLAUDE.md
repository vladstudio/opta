...
Opta — a macOS SwiftUI app that optimizes images. Accepts PNG, JPEG, TIFF, GIF, BMP, HEIC, and WebP as input; outputs optimized PNG or WebP. Non-PNG inputs are converted to PNG via macOS built-in `sips` before processing. It shells out to CLI tools (`pngquant`, `oxipng`, `cwebp`) installed via Homebrew. No third-party Swift dependencies.
...
- **Models.swift** — `OutputFormat` enum (PNG/WebP), `FileStatus` enum, `FileItem` observable model, `colorSteps` array for quantization levels
- **ProcessingEngine.swift** — `ObservableObject` that runs the optimization pipeline on a background `DispatchQueue`: convert non-PNG to PNG via sips → quantize via pngquant → optimize via oxipng (PNG) or convert via cwebp (WebP). Uses `Process` to shell out to CLI tools. Supports cancellation via lock-protected flag.
